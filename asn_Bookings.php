...
if(isset($_POST['search'])) {
  ...
  if($res && $res->num_rows) {
    ...
    $statusMsg = "⚠️ No booking found with ID $id.";
  }
}
...
if(isset($_POST['update'])) {
  ...
  if($upd->execute()) {
    $statusMsg = "✅ Updated status for booking #$id to “$status.”";
  } else {
...
if(isset($_POST['delete'])) {
  $id  = (int)$_POST['id'];
  $res = $conn->query("SELECT id FROM booking WHERE id=$id");
  if($res && $res->num_rows) {
    $conn->query("DELETE FROM booking WHERE id=$id");
    $statusMsg = "🗑️ Booking #$id deleted.";
  // Renumber IDs to be sequential
  $conn->query("SET @num := 0");
  $conn->query("UPDATE booking SET id = (@num := @num + 1) ORDER BY id");
  $conn->query("ALTER TABLE booking AUTO_INCREMENT = 1");
  } else {
    $statusMsg = "⚠️ No booking #$id to delete.";
  }
}

// 5) REPORT
if(isset($_POST['report'])) {
  $reportQ = $conn->query("SELECT * FROM booking");
  if(!$reportQ || !$reportQ->num_rows) {
    $statusMsg = "ℹ️ No bookings to display.";
    $reportQ   = null;
  }
}
...
<head>
  ...
  <title>Admin Panel • Tour Status</title>
...
<body>
  ...
  <main>
    ...
    <!-- Manage Tour Status -->
    <section class="card">
      <h2>Manage Tour Details </h2>
      <form method="post">
        <div class="form-group">
          <label>Booking ID</label>
          <input type="text" name="id" class="md-input" placeholder="Enter booking ID…" />
        </div>
            ...
            <?php while($r = $reportQ->fetch_assoc()): ?>
              <tr>
                <form method="post">
                  ...
                  <td><input type="text" name="start_date" value="<?= htmlspecialchars($r['start_date']) ?>" class="md-input" /></td>
                  <td><input type="text" name="end_date" value="<?= htmlspecialchars($r['end_date']) ?>" class="md-input" /></td>
                  <td><input type="number" name="passengers" value="<?= htmlspecialchars($r['passengers']) ?>" class="md-input" /></td>
                  <td><input type="text" name="room_option" value="<?= htmlspecialchars($r['room_option']) ?>" class="md-input" /></td>
                  <td><input type="text" name="optional_tours" value="<?= htmlspecialchars($r['optional_tours']) ?>" class="md-input" /></td>
                  <td><input type="number" name="total" value="<?= htmlspecialchars($r['total']) ?>" class="md-input" /></td>
                  <td>
                    <select name="status" class="md-select">
                      <option value="Pending"<?= (isset($r['status']) && $r['status']==='Pending')?' selected':''; ?>>Pending</option>
                      <option value="Confirmed"<?= (isset($r['status']) && $r['status']==='Confirmed')?' selected':''; ?>>Confirmed</option>
                      <option value="In Progress"<?= (isset($r['status']) && $r['status']==='In Progress')?' selected':''; ?>>In Progress</option>
                      <option value="Completed"<?= (isset($r['status']) && $r['status']==='Completed')?' selected':''; ?>>Completed</option>
                      <option value="Canceled"<?= (isset($r['status']) && $r['status']==='Canceled')?' selected':''; ?>>Canceled</option>
                    </select>
                  </td>
