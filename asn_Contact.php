...
if(isset($_POST['search'])) {
  $id   = (int)$_POST['id'];
  $res  = $conn->query("SELECT * FROM contact_as WHERE id=$id");
  if($res && $res->num_rows) {
    $searchRec = $res->fetch_assoc();
  } else {
    $statusMsg = "⚠️ No contact found with ID $id.";
  }
}
...
if(isset($_POST['update'])) {
  ...
  $upd = $conn->prepare("UPDATE contact_as SET name=?, email=?, subject=?, message=?, status=? WHERE id=?");
  $upd->bind_param("sssssi", $name, $email, $subject, $message, $status, $id);
  if($upd->execute()) {
    $statusMsg = "✅ Updated contact #$id.";
  } else {
...
if(isset($_POST['delete'])) {
  $id  = (int)$_POST['id'];
  $res = $conn->query("SELECT id FROM contact_as WHERE id=$id");
  if($res && $res->num_rows) {
    $conn->query("DELETE FROM contact_as WHERE id=$id");
    $statusMsg = "🗑️ Contact #$id deleted.";
  // Renumber IDs to be sequential
  $conn->query("SET @num := 0");
  $conn->query("UPDATE contact_as SET id = (@num := @num + 1) ORDER BY id");
  $conn->query("ALTER TABLE contact_as AUTO_INCREMENT = 1");
  } else {
    $statusMsg = "⚠️ No contact #$id to delete.";
  }
}

// 5) REPORT
if(isset($_POST['report'])) {
  $reportQ = $conn->query("SELECT * FROM contact_as");
  if(!$reportQ || !$reportQ->num_rows) {
    $statusMsg = "ℹ️ No contacts to display.";
    $reportQ   = null;
  }
}
  ...
  <main>
    ...
    <section class="card">
      ...
      <form method="post">
        <div class="form-group">
          ...
          <input type="text" name="id" class="md-input" placeholder="Enter contact ID…" />
        </div>
    ...
    <?php if($searchRec): ?>
      <section class="card">
        ...
        <p><strong>Name:</strong> <?= htmlspecialchars($searchRec['name']) ?></p>
        <p><strong>Email:</strong> <?= htmlspecialchars($searchRec['email']) ?></p>
        <p><strong>Subject:</strong> <?= htmlspecialchars($searchRec['subject']) ?></p>
        <p><strong>Message:</strong> <?= htmlspecialchars($searchRec['message']) ?></p>
      </section>
    <?php endif; ?>
            ...
            <?php while($r = $reportQ->fetch_assoc()): ?>
              <tr>
                <form method="post">
                  ...
                  <td><input type="text" name="subject" value="<?= htmlspecialchars($r['subject']) ?>" class="md-input" /></td>
                  <td><textarea name="message" class="md-input"><?= htmlspecialchars($r['message']) ?></textarea></td>
                  <td>
