...
if (isset($_POST["submit2"])) {
    ...
    $Package_type = "Scenic Sri Lanka Explorer";

    // Collect inputs
    $start_date = trim($_POST['start_date'] ?? '');
    $end_date = trim($_POST['end_date'] ?? '');
    $tripDays = (int)($_POST['tripDays'] ?? 8);
    ...
    $optionalToursSafe = [];
    if (is_array($optionalToursArr)) {
        foreach ($optionalToursArr as $it) $optionalToursSafe[] = mysqli_real_escape_string($con, $it);
    }
    ...
    if (empty($start_date) || empty($end_date) || $passengerInput < 1 || $name === '' || $email === '') {
        ...
        $base_price_per_person = 975.0; // $975 USD per person
    ...
    $stmt = mysqli_prepare($con, "INSERT INTO booking (`Package`,`start_date`,`end_date`,`passengers`,`room_option`,`optional_tours`,`base_price`,`extras`,`total`,`pay_on_arrival`,`name`,`email`,`special_request`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        if (!$stmt) {
...
<body>
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
        ...
        <div class="container-fluid bg-breadcrumb">
            <div class="container text-center py-5" style="max-width: 900px;">
                <h3 class="text-white display-3 mb-4" style="color: white;">Book the Scenic Sri Lanka Explorer</h3> 
        ...
        <div class="container"><br>
            <div class="section"><br>
                ...
                <select id="tripDays" disabled style="background-color: #e9ecef; cursor: not-allowed;">
                    <option value="8" selected>8 Day(s)</option>
                </select>
                <input type="hidden" name="tripDays" value="8">
    ...
    <script>
        ...
        function calculateEndDate() {
            const days = parseInt(document.getElementById('tripDays').value) || 8;
        ...
        function calculateTotal(event) {
            ...
            const basePricePerPassenger = 975; // $975 USD per person
        ...
        const STORAGE_KEY = 'asan_booking_scenic_explorer';
